...
export default function RegisterPage() {
  ...
  const animateSwitch = () => {
    Animated.parallel([
      ...
      Animated.timing(slideAnim, { toValue: -20, duration: 200, easing: Easing.out(Easing.ease), useNativeDriver: true }),
    ]).start(() => {
    ...
    try {
      if (isSignUp) {
        ...
        if (error) {
          ...
          setErrorMessage(error.message.includes('already registered') ? 'This email is already registered.' : error.message);
          setLoading(false);
          return;
        }

        if (data.user?.id) {
          const profileInsert: ProfilesInsert = {
        ...
        router.push({ pathname: '/(auth)/name/page', params: { email } });
  ...
  const isDisabled = isSignUp ? isEmpty || isInvalidEmail || !passwordValid : isEmpty || invalidCred;
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }], width: '100%' }}>
        <View style={styles.contentWrapper}>
          <Text style={[styles.title, { color: colors.text }]}>{isSignUp ? 'Create an Account' : 'Log in'}</Text>
          <Text style={[styles.subtext, { color: colors.text + 'AA' }]}>
            {isSignUp ? 'Join us to start decluttering with purpose.' : 'Continue your ClutterHuck journey.'}
          </Text>
          ...
          <View style={[styles.formCard, { backgroundColor: containerBg, borderColor: borderSoft }]}>
            <TextInput
              ...
              theme={{ roundness: 50, colors: { primary: focusColor, outline: borderSoft, text: colors.text, placeholder: colors.text + '88', background: colors.card } }}
            />
            ...
            <View style={{ position: 'relative' }}>
              <TextInput
                ...
                selectionColor={focusColor}
                cursorColor={focusColor}
                right={isSignUp && password.length > 0 ? (
                  <TextInput.Icon icon={() => (
                    <Ionicons
                      name={passwordValid ? 'checkmark-circle' : 'close-circle'}
                      size={20}
                      color={passwordValid ? (dark ? '#7CFC7C' : '#28a745') : passwordErrorColor}
                    />
                  )} />
                ) : null}
                theme={{ roundness: 50, colors: { primary: focusColor, outline: borderSoft, text: colors.text, placeholder: colors.text + '88', background: colors.card } }}
              ...
              {isSignUp && password.length > 0 && !passwordValid && (
                <Text style={[styles.passwordHint, { color: passwordErrorColor }]}>
            ...
            <View style={styles.switchContainer}>
              {isSignUp ? (
                <Text style={[styles.switchText, { color: fadedText }]}>
                  ...
                  <Text style={[styles.linkText, { color: colors.text, textDecorationLine: 'underline' }]} onPress={animateSwitch}>
                    Log In
                  </Text>
                </Text>
              ) : (
...
const styles = StyleSheet.create({
  ...
  brand: { position: 'absolute', top: 60, alignSelf: 'center', fontSize: 22, fontWeight: '800' },
  contentWrapper: { justifyContent: 'center', flexGrow: 1, marginTop: 60 },
  title: { fontSize: 26, fontWeight: 'bold', textAlign: 'center', marginBottom: 8 },
  subtext: { fontSize: 14, textAlign: 'center', marginBottom: 30 },
  formCard: { borderWidth: 1, borderRadius: 30, padding: 20, shadowColor: '#000', shadowOpacity: 0.05, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4 },
  input: { borderRadius: 50, marginBottom: 15 },
  passwordHint: { fontSize: 12, marginTop: -10, marginLeft: 15, marginBottom: 8 },
  button: { paddingVertical: 16, borderRadius: 50, alignItems: 'center', marginTop: 10 },
